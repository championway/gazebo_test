...
def call_back(msg):
    ...
    br.sendTransform((0.0, 0.0, 0.0),tf.transformations.quaternion_from_euler(0, 0, 0),rospy.Time.now(),"/map","/gazebo")
    # robot <---> map
    ...
    # lidar <---> map
    br.sendTransform((0.0, 0.0, 0.0),tf.transformations.quaternion_from_euler(0, 0, -np.pi/2.0),rospy.Time.now(),"/velodyne","robot")
